;;; package -- Summary
;;; Commentary:
;;; Code:
(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(bmkp-last-as-first-bookmark-file "~/.emacs.d/bmkp/bookmarks")
 '(package-selected-packages
   '(win-switch multi-term bm ace-window hlinum magit flycheck recentf-ext cider undohist undo-tree intero go-eldoc flymake-haskell-multi company-go company-ghc auto-highlight-symbol))
 '(safe-local-variable-values
   '((cider-default-cljs-repl . shadow)
     (cider-shadow-cljs-default-options . "app"))))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 ;; '(highlight ((t (:background "cyan" :foreground "black"))))
 ;; '(icicle-candidate-part ((t (:background "yellow" :foreground "black"))))
 ;; '(icicle-key-complete-menu ((t (:background "#EF84FFEAF427" :foreground "black" :box (:line-width 1 :color "#34F393F434F3")))))
 ;; '(icicle-key-complete-menu-local ((t (:background "#EF47FFFFC847" :foreground "black" :box (:line-width 1 :color "#AC4AAC4A0000")))))
 ;; '(icicle-proxy-candidate ((t (:background "#E1E1EAEAFFFF" :foreground "black" :box (:line-width 2 :color "White" :style released-button)))))
 ;; '(icicle-saved-candidate ((t (:background "gray80" :foreground "black"))))
 ;; '(icicle-search-context-level-1 ((t (:background "#FA6CC847FFFF" :foreground "black"))))
 ;; '(icicle-search-context-level-2 ((t (:background "#C847FFFFE423" :foreground "black"))))
 ;; '(icicle-search-context-level-3 ((t (:background "#C847D8FEFFFF" :foreground "black"))))
 ;; '(icicle-search-context-level-4 ((t (:background "#EF47FFFFC847" :foreground "black"))))
 ;; '(icicle-search-context-level-5 ((t (:background "#FCFCE1E1FFFF" :foreground "black"))))
 ;; '(icicle-search-context-level-6 ((t (:background "#E1E1FFFFF0F0" :foreground "black"))))
 ;; '(icicle-search-context-level-7 ((t (:background "#E1E1EAEAFFFF" :foreground "black"))))
 ;; '(icicle-search-context-level-8 ((t (:background "#F6F5FFFFE1E1" :foreground "black"))))
 ;; '(icicle-search-main-regexp-current ((t (:background "magenta" :foreground "black"))))
 ;; '(icicle-search-main-regexp-others ((t (:background "cyan" :foreground "black"))))
 ;; '(icicle-special-candidate ((t (:foreground "red"))))
 ;; '(info-index-match ((t (:inherit match :foreground "black"))))
 ;; '(isearch-fail ((t (:background "magenta"))))
 ;; '(lazy-highlight ((t (:background "paleturquoise" :foreground "black"))))
 ;; '(linum ((t (:foreground "white" :background "color-234"))))
 ;; '(linum-highlight-face ((t (:foreground "black" :background "red"))))
 ;; '(tab-bar ((t (:inherit variable-pitch :background "black" :foreground "green"))))
 ;; '(tab-bar-tab ((t (:inherit nil :box (:line-width 1 :color "blue" :style pressed-button)))))
 ;; '(tab-bar-tab-inactive ((t (:inherit tab-bar-tab :foreground "color-244" :background "black"))))
 )

;;; custom.el ends here
